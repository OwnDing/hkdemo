...
import static com.zhite.zhite.service.HkClientCamera.hCNetSDK;
import static com.zhite.zhite.service.HkClientCamera.playControl;

@Component
public class HkVideoPlay {
    ...
    // 注入日志记录器
    private static final Logger logger = LoggerFactory.getLogger(HkVideoPlay.class);
    ...
    public static int getRealStreamData(int userID, int iChannelNo) {
        ...
        if (Handle == -1) {
            ...
            logger.error("取流失败, USERID:{}, CHANNELNO:{}", userID, iChannelNo);
            return -1;
        }
        System.out.println("取流成功");
        logger.info("取流成功, USERID:{}, CHANNELNO:{}", userID, iChannelNo);
        return Handle;
    }
    ...
    public static int getESRealStreamData(int userID, int iChannelNo) {
        ...
        if (!esRealDatafile.exists()) {
            try {
                ...
                e.printStackTrace();
                logger.error("创建文件失败, USERID:{}, ERROR:{}", userID, e);
            }
        }
        try {
        ...
        if (Handle == -1) {
            int iErr = hCNetSDK.NET_DVR_GetLastError();
            System.err.println("取流失败" + iErr);
            logger.error("取流失败, USERID:{}, ERRORCODE:{}", userID, iErr);
            return -1;
        }
        System.out.println("取流成功");
        ...
        if (!hCNetSDK.NET_DVR_SetESRealPlayCallBack(Handle, fPlayescallback, null)) {
            ...
            logger.error("设置裸码流回调失败，错误码：", hCNetSDK.NET_DVR_GetLastError());
        }
...
/*            switch (dwDataType) {
    ...
    /**
     *  回调功能修改可参考 https://blog.csdn.net/qq_16504067/article/details/118102500
     *                  https://blog.csdn.net/eguid_1/article/details/108755316
     */
